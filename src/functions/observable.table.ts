...
class ObservableTable extends Subject<any> {
	...
	constructor(orm: any, entity: any, listener: PostgresListener) {
		...
		listener.notifications //
			.pipe(filter((notification: PostgresNotificationType): boolean => notification?.table === this._tableName))
			.subscribe({
				next: (notification: PostgresNotificationType): void => {
	...
	private async _process(notification: PostgresNotificationType): Promise<void> {
