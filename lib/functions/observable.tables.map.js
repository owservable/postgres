...
class PostgresObservableTablesMap {
    static init() {
        if (!PostgresObservableTablesMap._instance)
            PostgresObservableTablesMap._instance = new PostgresObservableTablesMap();
        return PostgresObservableTablesMap._instance;
    }
    static get(orm, entity, listener) {
        const instance = PostgresObservableTablesMap.init();
    ...
    static clear() {
        if (PostgresObservableTablesMap._instance)
            PostgresObservableTablesMap._instance._map.clear();
    }
    constructor() {
        this._map = new Map();
    }
}
exports.default = PostgresObservableTablesMap;
