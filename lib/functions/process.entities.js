...
const _processPostgresEntities = (folder, entities, exclude) => {
    ...
    folders.forEach((subFolder) => _processPostgresEntities(subFolder.fullPath, entities, exclude));
};
const processPostgresEntities = (root, name = 'entities', exclude) => {
    const entities = [];
    const folders = (0, folders_1.listSubfoldersByName)(root, name);
    folders.forEach((folder) => _processPostgresEntities(folder, entities, exclude));
    return entities;
};
exports.default = processPostgresEntities;
