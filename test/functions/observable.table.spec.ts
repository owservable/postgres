...
describe('observable.table tests', () => {
	...
	it('should emit delete notifications without fetching the entity', async () => {
		...
		listener.notifications.next({table: 'users', op: 'delete', id: '3'});
		await flush();
	...
	it('should fetch and emit the full document on insert with a coerced numeric pk', async () => {
		...
		listener.notifications.next({table: 'users', op: 'insert', id: '4'});
		await flush();
	...
	it('should fetch using the raw string id when the pk is not numeric', async () => {
		...
		listener.notifications.next({table: 'users', op: 'insert', id: 'abc'});
		await flush();
	...
	it('should emit without a full document when the entity is not found', async () => {
		...
		listener.notifications.next({table: 'users', op: 'update', id: '9'});
		await flush();
	...
	it('should map changed columns to properties in the update description', async () => {
		...
		listener.notifications.next({table: 'users', op: 'update', id: '4', changed: ['first_name', 'unknown_col']});
		await flush();
	...
	it('should build an empty update description when no changed columns are provided', async () => {
		...
		listener.notifications.next({table: 'users', op: 'update', id: '4'});
		await flush();
	...
	it('should ignore notifications for other tables and empty notifications', async () => {
		...
		listener.notifications.next(undefined);
		listener.notifications.next({table: 'other', op: 'insert', id: '1'});
		await flush();
	...
	it('should emit an empty change on live lifecycle events after the first one', () => {
		...
		listener.lifecycle.next(undefined);
		listener.lifecycle.next({type: 'close'});
		listener.lifecycle.next({type: 'live'});
		expect(emissions).toEqual([]);
	...
	it('should log processing errors', async () => {
		...
		listener.notifications.next({table: 'users', op: 'insert', id: '1'});
		await flush();
