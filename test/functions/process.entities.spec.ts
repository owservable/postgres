...
import processPostgresEntities from '../../src/functions/process.entities';
...
describe('process.entities tests', () => {
	it('should collect entities recursively using the default folder name', () => {
		const entities: any[] = processPostgresEntities(path.join(fixtures, 'good'));
	...
	it('should exclude folders matching a string suffix', () => {
		const entities: any[] = processPostgresEntities(path.join(fixtures, 'good'), 'entities', 'skipme');
	...
	it('should exclude folders matching any suffix in an array', () => {
		const entities: any[] = processPostgresEntities(path.join(fixtures, 'good'), 'entities', ['skipme', 'skipme2']);
	...
	it('should keep all entities when the array exclude matches nothing', () => {
		const entities: any[] = processPostgresEntities(path.join(fixtures, 'good'), 'entities', ['nomatch']);
		expect(names(entities)).toEqual(['HelperEntity', 'NoteEntity', 'Skipped2Entity', 'SkippedEntity', 'UserEntity']);
	});

	it('should return nothing when the entities folder itself is excluded', () => {
		const entities: any[] = processPostgresEntities(path.join(fixtures, 'good'), 'entities', 'entities');
		expect(entities).toEqual([]);
	});

	it('should throw when a file has no default export', () => {
		expect(() => processPostgresEntities(path.join(fixtures, 'bad'))).toThrow(/Entity not found in/);
	});

	it('should return an empty array when no folders match the name', () => {
		expect(processPostgresEntities(path.join(fixtures, 'good'), 'nonexistent')).toEqual([]);
	});
});
